    ...
    protected static function booted()
    {
        parent::created(function (User $user) {
            ...
            if(auth()->check() && auth()->user()->hasRole('admin')) {
                if(isset($user->id)) {
                    \Helper::CreateReport('Criou', 'Usuário criado pelo admin: '. auth()->user()->name . ' o id é:' .$user->id);
                }
            }
        }); 
...
{
    return $this->last_activity && $this->last_activity->diffInMinutes(now()) < 5; // Define 5 minutos como o limite
}
