...
public function GameLaunchDrakon(Game $game)
{
    if ($token = self::DrakonAuthentication()) {
        $gateway = GamesKey::first();
        if (!empty($gateway)) {
            $user = auth('api')->user(); // Obtém o usuário autenticado
            $userName = $user->name; // Obtém o nome do usuário
    ...
    if (isset($request->user_id)) {
        ...
        if (!empty($user) && !empty($wallet)) {
            ...
            if ($changeBonus != 'no_balance') {
                $hashGenerate = Helper::GenerateHash('hash:' . $request->round_id . '#' . $request->game . '#' . $request->user_id);
    ...
    if (isset($request->user_id)) {
        ...
        if (!empty($user) && !empty($wallet)) {
            ...
            if (!empty($checkTransaction)) {
                ...
                Log::info('CreateDrakonTransactions returned', ['transactionCreated' => $transactionCreated]);

                if (!empty($transactionCreated)) {
                    if ($win >= 0) {
                        Log::info('Calling generateGameHistory for user_id: ' . $wallet->user_id);
                        ...
                        $historyGenerated = Helper::generateGameHistory(
                            ...
                            $bet,
                            $checkTransaction->type_money,
                            $checkTransaction->round_id
                        );

                        Log::info('generateGameHistory returned: ' . ($historyGenerated ? 'true' : 'false'));

                        if (!$historyGenerated) {
                            Log::error('generateGameHistory failed for user_id: ' . $wallet->user_id . ' with bet: ' . $bet . ' and win: ' . $win);

                            Log::info('Incrementing wallet balance for user_id: ' . $wallet->user_id . ' by amount: ' . $win);
                            ...
                            Log::info('generateGameHistory successful for user_id: ' . $wallet->user_id);
                        }
...
{
    \Log::info('CreateDrakonTransactions called with', [
        'player_id' => $playerId,
        'bet_reference_num' => $betReferenceNum,
    ...
    {
        Log::info('SetRefundDrakon', ['request' => $request]);
        if (isset($request->user_id)) {
            $user = User::with(['wallet'])->find($request->user_id);
            $wallet = Wallet::where('user_id', $user->id)->first();
            ...
            if (!empty($user) && !empty($wallet)) {
              	...
              	$isTransactionToRefund = Order::where('transaction_id', $request->transaction_id)->first();
              
              	if (!empty($isTransactionToRefund)) {
                  		$isTransactionToRefund->refunded = true;
                  		$isTransactionToRefund->save();
                  
                        if ($isTransactionToRefund->type === 'bet') {
                			$wallet->increment($isTransactionToRefund->type_money, $request->amount);
                        } else {                    
	                		$wallet->decrement($isTransactionToRefund->type_money, $request->amount);
                        }
                    ...
                    $checkTransaction = Order::where('round_id', $request->round_id)
                        ->where('type', 'bet')
                        ->where('hash', $hashGenerate)
                        ->first();
                    ...
                    if (!empty($checkTransaction)) {
                        ...
                        $wallet->increment($checkTransaction->type_money, $request->amount);

                        Log::info('SetRefundDrakon - Response', [
                            'status' => true,
                            'balance' => $wallet->total_balance,
                        ]);
                        return response()->json([
